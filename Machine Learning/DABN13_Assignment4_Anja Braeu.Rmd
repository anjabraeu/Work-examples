...
``` {r }
PCscores.1c    <- cbind(X.1a %*% X.svd.1b$v[,1], X.1a %*% X.svd.1b$v[,2])
...
pcr.fit.1e     <- pcr(mkt.ret.12m ~ ., data= data.1e, scale=T, center=T, ncomp=2)
pcr.pred.1e    <- as.matrix(predict(pcr.fit.1e, data.1e, ncomp=2))
diff.pred.1e   <- sum((pcr.pred.1d - pcr.pred.1e)^2)
```
...
n.obs.2a  <- nrow(portfolios)
n.rwin.2a <- 120
train.2a  <- cbind(1:(n.obs.2a-n.rwin.2a),n.rwin.2a:(n.obs.2a-1))
holdout.2a <- cbind((n.rwin.2a+1):n.obs.2a,(n.rwin.2a+1):n.obs.2a)
```
...
Mmax <- 5
pcr.fit.2c      <- pcr(y ~ ., data= data.train.2b, scale=T, center=T, ncomp=Mmax)
pcr.pred.2c     <- as.matrix(predict(pcr.fit.2c, data.holdout.2b, ncomp=1:Mmax))
...
for (m in 1:Mmax) {
  CVloss.all.2d[j,m] <- sum((data.holdout.2b$y - pcr.pred.2c[,,m])^2)
}
```
...
cv.pcr <- function(X,y, Mmax, train, holdout) {
  ...
  num.itr       <- nrow(data.train)
  CVloss.all <- matrix(NA, nrow=num.itr, ncol=Mmax)


  for (j in 1:num.itr) {
  data.train   <- data[train[j,1]:train[j,2],]
  data.holdout <- data[holdout[j,1]:holdout[j,2],]
  pcr.fit      <- pcr(y ~ ., data= data.train, scale=T, center=T, ncomp=Mmax)
  pcr.pred <- predict(pcr.fit, newdata=data.holdout, ncomp=1:Mmax)
    
    for (m in 1:Mmax) {
      CVloss.all[j,m] <- sum((data.holdout$y - pcr.pred[,,m])^2)
    }
...
cv.plsr <- function(X,y, Mmax, train, holdout) {
  ...
  data.train   <- data[train[j,1]:train[j,2],]
  data.holdout <- data[holdout[j,1]:holdout[j,2],]
  plsr.fit <- plsr(y ~ ., data= data.train, scale=T, center=T, ncomp=Mmax)
  plsr.pred <- predict(plsr.fit, newdata=data.holdout, ncomp=1:Mmax)
    
    for (m in 1:Mmax) {

      CVloss.all[j,m] <- sum((data.holdout$y - plsr.pred[,,m])^2)
    }
...
pcr.cv.2h <- cv.pcr(portfolios, mkt.ret.12m, Mmax=15, train.2a, holdout.2a)
pls.cv.2h <- cv.plsr(portfolios, mkt.ret.12m, Mmax=15, train.2a, holdout.2a)
which.nums.chosen.2h <-"For PCR, 13 directions  are chosen, for PLS 4 are chosen. These numbers of directions (i.e. variables) produces the minimal mean CVloss."
...
n.obs.3a  <- nrow(portfolios)
n.init.3a <- 450
train.3a  <- cbind(1:(n.obs.3a-n.init.3a),n.init.3a:(n.obs.3a-1))
test.3a   <- cbind((n.init.3a+1):n.obs.3a,(n.init.3a+1):n.obs.3a)
```
...
why.expand.train.set3b = "When working with time series data, information/data changes over time, thus, the relationship between the predictors X and the outcome y changes over time as well. Therefore, it makes more sense to fit the model on data that is from a timeperiod that is relatively close to the holdout/test set, instead of extending it to all available data from all previous periods. However, one needs to consider that this leads to a trade-off between the number of observations available for training and up-to-date data. However, choosing to extend the training set to a greater timeperiod meand risking that it contains outdated relationships between X and y, ultimately leading to a worse model fit. A smaller, more up-to-date trainingset may provide more accurate predictions."
...
pcr.pred.3c <- c(rep(NA, nrow(test.3a)))
pls.pred.3c <- c(rep(NA, nrow(test.3a)))
...
# Define cross-validation samples
  ...
  pcr.cv.3d  <- cv.pcr(portfolios, mkt.ret.12m, Mmax=15, train.cv, holdout.cv)
  pcr.mhat   <- which.min(pcr.cv.3d[,2])
  pls.cv.3d  <- cv.plsr(portfolios, mkt.ret.12m, Mmax=15, train.cv, holdout.cv)
  pls.mhat   <- which.min(pls.cv.3d[,2])

  # Fit tuned model and get predictions
  pcr.fit.3d          <- pcr(y ~ ., data= data.3a.train, scale=T, center=T, ncomp=pcr.mhat)
  pcr.pred.3c[itr]    <- predict(pcr.fit.3d, newdata= data.3a.test, ncomp=pcr.mhat)

  pls.fit.3d          <- plsr(y ~ ., data= data.3a.train, scale=T, center=T, ncomp=pls.mhat)
...
y.test.3e <- data.2b[451:600, 1]
TSS       <- t(y.test.3e - mean(y.test.3e)) %*% (y.test.3e - mean(y.test.3e))
pcr.R2    <- 1 - sum((y.test.3e - pcr.pred.3c)^2)/TSS
pls.R2    <- 1 - sum((y.test.3e - pls.pred.3c)^2)/TSS
my.pref.methd.3e <- "The preferred method is the one with a higher R2. PCR has an R2 of 0.15 and PLS and R2 of 0.43. Thus, PLS is clearly the preferred method for forecasting twelve-month market returns in the future. The reason for this is that the number of chosen directions differs: PCR chooses 15 directions, PLS 5. With 15 variables to fit, PCR creates more noise in the data than PLS, which results in a worse model fit than PLS."
