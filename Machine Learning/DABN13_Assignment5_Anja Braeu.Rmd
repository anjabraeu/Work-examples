...
library(tidyverse)
alpha=0.05
p.holm <-  p.adjust(p.values, "holm")
names.selected <- names(which(p.holm<=alpha))[-1] 
```
...
p <- pt(-t, nrow(X)-ncol(X), lower.tail = TRUE) + pt(t, nrow(X)-ncol(X), lower.tail = FALSE)
...
```{r }
calculate.p <- function(y, X, df, solve.X, solve.beta){ # add the extra variables
  beta.hat <- solve.beta %*% y
  sigma2.hat <- sum((y-X%*%beta.hat)^2)/df
  VX <- diag(sigma2.hat*solve.X)
  t <- abs(beta.hat/sqrt(VX))
  p.values <- pt(-t, df, lower.tail = TRUE) + pt(t, df, lower.tail = FALSE)
return(p.values)
}
...
df = nrow(X)-ncol(X)
solve.X = solve(t(X) %*% X)
solve.beta = solve.X%*%t(X)
start.time <- Sys.time()
for(i in 1:n.sim){
  y.fake <- rnorm(length(y), mean(y), sd(y)) #var or sd
  .vals <- calculate.p(y.fake, X, df, solve.X, solve.beta)
   p.holm <- p.adjust(.vals, "holm")[-1]
   p.BF   <- p.adjust(.vals, "bonferroni")[-1]
 n.selected.Holm[i] <- sum(p.holm<=alpha)
 n.selected.BF[i]   <- sum(p.BF<=alpha)
}
end.time <- Sys.time()
cat(n.sim, 'iterations took' , (end.time-start.time), 'sec')
FWER.BF.2d <- sum(n.selected.BF>0)/n.sim
FWER.Holm.2d <- sum(n.selected.Holm>0)/n.sim

task2f.dotheycontrolFWER <- "Both Bonferroni and Holm are successful at controlling the FWER."
```
...
for( i in 1:n.sim){
   ...
   lm.obj.fake <- lm(log(BOX) ~ ., data.fake)
   p.holm <- p.adjust(summary(lm.obj.fake)$coefficients[,4], "holm")[-1]
   p.BF    <- p.adjust(summary(lm.obj.fake)$coefficients[,4], "bonferroni")[-1]
   n.selected.Holm[i] <- sum(p.holm<=alpha)
   n.selected.BF[i]   <- sum(p.BF<=alpha)
}
end.time <- Sys.time()
cat(n.sim, 'iterations took' , (end.time-start.time), 'sec')
quicker.2e <- "Our own function is considerably quicker and is more efficient. There are two reasons for this difference: Firstly, the lm() function computes much more than just the p-values (e.g. the coefficients, t-statistic, etc.), whereas our function just calculates the p-values. Secondly, our function does not have to compute the fixed components every time within the loop, since we optimized it for efficiency by taking these out of the loop, whereas the lm() function has to do that over and over again, which costs time."
```
...
solve.X = solve(t(X.scaled) %*% X.scaled)
solve.beta = solve.X%*%t(X.scaled)
```



## Task 3b)
Next, we simulate fake outcomes. However, in contrast to task 2g we are not creating new $Y$ that are completely unrelated to the observed predictors in our box office sales data. More specifically, we simulate $Y$ from the linear model
$$
...
beta.fake[signal.index] <- log(2:5)
y.fake <- X.scaled %*% beta.fake + rnorm(nrow(X.scaled),0,1)
```
...
```{r }
alpha = 0.15
p.vals       <- calculate.p(y.fake, X.scaled, df, solve.X, solve.beta)
...
fdp.BF   <-  (sum(selected.BF) - sum(selected.true.BF))/sum(selected.BF)
fdp.hochberg    <-  (sum(selected.hochberg) - sum(selected.true.hochberg))/sum(selected.hochberg)
...
Task3d.why.difference <- "There are several signal variables, of which one is the strongest and one the weakest. The strongest signal will likely have the smallest p-value among all signals, whereas the weakest signal will have p-values that are on average* bigger than the p-values of the strongest signal. For both methods, the p-values are ordered from the smallest to highest, meaning for our example that the p-values of the strongest signal will be on the 1st position, while the p-values of the weakest signal will likely be on the 4th position.
If we now look at the threshold for rejecting or not rejecting the Null-Hypotheses, the decision boundary of the Hochberg method increases with every position, meaning that at the 4th position, a Null-Hypothesis with a higher p-value will be accepted than at the 1st position. The threshold for Bonferroni on the other holds constant at a/N for p-values at every position. The decision boundaries at the 1st position, where the p-values of the strongest signal is located, is very similar for both methods and since the p-values are comparably low, the Null-Hypothesis for the strongest signal will most likely be rejected by both methods. With increasing magnitude, the coefficients of the signals are scaled up, meaning that the p-values decrease even more and making it more likely to reject the Null-Hypothesis for the strongest signal by both methods. Since the Power is an indication of how good we are at rejecting the right Null-Hypotheses, it will become greater with increasing magnitude.
Now looking at the 4th position, where the p-value of the weakest signal is located, the thresholds of the Hochberg method is higher (more allowing) than of the Bonferroni method. Thus, the Hochberg method rightfully rejects the Null-Hypothesis with a greater likelhood and at a smaller magnitude than the Bonferroni method.
In short, the greater difference at the weakest signal has to do with the ordering of the p-values and the differences in the thresholds of both methods at different positions.

*It is to note that we are doing 1000 iterations and will have p-value clouds instead of only one p-value. "
...
Task3d.what.pattern<- "The FWER of Bonferroni is a horizontal line (mostly constant), while the FWER of Hochberg increases with increasing magnitude."



Task3d.why.pattern<- "The increasing FWER with increasing magnitude of the Hochberg method results from the reordering of the p-values when the coefficients of the signals get scaled up and thus the p-values of the signals get smaller (see 3d). The noise variables are not affected by the magnitude. So if we for example order the p-values of signal and noise variables from smallest to biggest, a noise variable may be on a position in front of a signal, before we scale the coefficients up. As established in 3d), the Hochberg method has an increasing threshold for rejecting Null-Hypotheses. Now, if we scale up the coefficient of a weak signal, it's p-value might get smaller than the one of the noise variable. Then, they switch places and the threshold for the Null-Hypothesis of the noise variable is higher than before - it might falsely get rejected now. We remember: the FWER is the probability of rejecting at least one true Null-Hypothesis.
Because of this reordering of the p-values when the magnitude gets bigger, the FWER of the Hochberg increases with increasing magnitude. But why does the FWER of the Bonferroni method mostly stay constant? Recall that the threshold of the Bonferroni method for rejecting Null-Hypotheses stays constant, and since the p-values of the noise variables don't change, no Null-Hypothesis of a noise variable will be rejected with increasing magnitude if it hasn't been rejected before.
We can conclude that the Bonferroni method controls the FWER, whereas the Hochberg does not (it controls the FDR, which is the fraction of the Null-Hypotheses that are falsely rejected).*

Again, it is to note that we are talking about p-value clouds, when we are talking about a p-value in this example, which is done for the sake of simplicitly."
```
## Task 4)
...
```{r }
library(glmnet)
glm.obj.lasso <- cv.glmnet(X, y, alpha=1 , intercept=F)
beta.coeff <-   coef(glm.obj.lasso, s = "lambda.1se")
...
library(knockoff)
fdr <- 0.15
result <- knockoff.filter(X, y, knockoffs = create.fixed, statistic = stat.glmnet_lambdasmax, offset=0, fdr=fdr)
