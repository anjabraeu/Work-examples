...
``` {r, echo=T}
 ...
 explain2inputs1c <- "The first input is a vector of the inital values for the parameters to be optimized over. The second input is then the function that is to be mini- or maximized."
...
``` {r, echo=T}
 minishoppers <- data.frame("Revenue" = y, "ER" = X[,"ER"], "lPR_Dur"= X[,"lPR_Dur"])
 glm.fit2a    <- glm(Revenue ~ ER + poly(lPR_Dur, degree=2, raw=TRUE), data=minishoppers, family= binomial)
 
 betahat2a    <- glm.fit2a$coefficients
 print(betahat2a)
...
``` {r, echo=TRUE}
 ...
 train <- sample(nobs[1], nobs[1]/2)
#or: sample(1:nobs[1], nobs[1]/2)
```
...
``` {r, echo=TRUE}
 glm.fit2c <- glm(Revenue ~ ER + poly(lPR_Dur, degree=2, raw=TRUE), data=minishoppers[train,], family= binomial)
...
``` {r, echo=TRUE}
 ...
 predictglm_type2d = "For the default binomial model, glm() predicts probability on the logit scale, i.e. log-odds. When specifying type='response', the predicted probabilities are given. The log-odds are the logit of the conditional probability that the outcome variable equals one (log(p(Y=1|X))) given the value of the predictors over the probability that the outcome equals zero(log(p(Y=0|X))). Type='response' gives the probability p(Y=1|X) over p(Y=0|X), i.e. not logit transformed."
...
``` {r echo=TRUE}
 ...
 glm.pred2e     <- vector("logical", length(glm.prob2d)) 
 glm.pred2e[glm.prob2d>0.5] <- TRUE

 logical2e      <- "glm.prob2d>0.5"
...
``` {r, echo=TRUE}
 ...
 testerr2f    <- sum(minishoppers[-train,]$Revenue != glm.pred2e)/length(glm.pred2e)
...
``` {r, echo=TRUE}
 performancemetrics <- function(ROCRpred, cutoff) {
   ...
   FPR   <- cmat[1,2]/(cmat[1,1] + cmat[1,2]) ## nochmal neu, weil fp und tp vertauscht
   TPR   <- cmat[2,2]/(cmat[2,1] + cmat[2,2])
   error <- (cmat[2,1]+cmat[1,2])/(cmat[1,1]+cmat[2,2]+cmat[2,1]+cmat[1,2])
...
``` {r, echo=TRUE}
 ...
 overall_acc_verdict3c      <- "The error rate with which the model predicts purchases is about 15%. For this industry, the overall accuracy is acceptable. Since the model predicts purchases better than taking the average, we can be satisfied, however, the accuracy could be better."
 obs_purchase_acc_verdict3c <- "Taking a look at the confusion matrix, the number of true positives is very low. Looking at the TPR, it is very close to zero, meaning that the model almost never predicts actual purchases right. Thus, the accuracy with which observed purchases are correctly predicted is very unsatisfactory and the model needs to be improved." 
...
``` {r, echo=TRUE}
 ...
 whereis_3bcombo3d <- "The combination of TPR and FPR from 3b) can be seen at (0,0). Here, the plot takes a yellow color, indicating the defined cutoff of 0.5." 
 optimal_cutoff3d  <- "To get a balanced class-specific performance, the optimal threshold should be about 0.18. At this point, FPR is approximately 0.34 and TPR 0.66."
```
...
accdata3e <- performance(glm.rocrpred3a,"acc")
 plot(accdata3e)
 is_accuracy_compromised3e <- "The chosen threshold compromises accuracy considerably. When changing the cutoff from 0.5 to 0.18, the accuracy drops from 85% to 65%, i.e. the error rate increases from 15% to 35%. Compromised accuracy however gives us a more balanced performance of the model, which the increased TPR shows. " 
...
``` {r, echo=TRUE}
 ...
 summarizeplot4a <- "The model predicts a purchase for exit rates that are close (for medium timespan) to zero or relatively small (for small and long timespan). Concerning the time that visitors spend on the product-related site, the model predicts that some people buy the product after a small timespan, but that the majority of purchases is made after a medium to long timespan. This could be interpreted that some people already know that they wanted to buy this specific product and thus not need to spend a long time on the page, whereas most people want to read the product specifications, possible reviews, etc. before they purchase."
...
``` {r, echo=TRUE}
 ...
 explaindecboundary4b <- "The decision boundaries of logistic regression are linear, when the predictors is linear. However, the fitted model within this assignment contains a second-order polynomial, which allows for nonlinear boundaries. Thus, the polynomial term in the predictors is the reason that we see a quadratic shape (curve) in the graph. " 
